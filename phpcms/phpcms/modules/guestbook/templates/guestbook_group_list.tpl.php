...
<form name="myform" action="?m=guestbook&c=guestbook&a=delete_group" method="post" onsubmit="checkuid();return false;">
<div class="table-list">
<table width="100%" cellspacing="0">
	<thead>
		<tr>
			<th width="35" align="center"><input type="checkbox" value="" id="check_box" onclick="selectall('guestbook_group_id[]');"></th>
...
if(is_array($infos)){
	...
	<tr>
		<td align="center" width="35"><input type="checkbox"
			name="guestbook_group_id[]" value="<?php echo $info['id']?>"></td>
		...
		<td align="center" width="10%"> <a
			href='?m=guestbook&c=guestbook&a=delete_group&guestbook_group_id=<?php echo $info['id']?>'
...
function see_all(id, name) {
	...
	window.top.art.dialog({title:'<?php echo L('details');//echo L('edit')?> '+name+' ',id:'edit',iframe:'?m=guestbook&c=guestbook&a=see_all&guestid='+id,width:'700',height:'450'}, function(){var d = window.top.art.dialog({id:'see_all'}).data.iframe;var form = d.document.getElementById('dosubmit');form.click();return false;}, function(){window.top.art.dialog({id:'see_all'}).close()});
}
function checkuid() {
	var ids='';
	$("input[name='guestbook_group_id[]']:checked").each(function(i, n){
		ids += $(n).val() + ',';
	});
	if(ids=='') {
