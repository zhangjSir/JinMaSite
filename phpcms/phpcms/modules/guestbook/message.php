...
class guestbook extends admin {
	function __construct() {
		...
		$this->group_db = pc_base::load_model('guestbook_group_model');
	...
	public function init() {
		...
		$infos = $this->db->listinfo($where,$order = 'guestid DESC',$page, $pages = '12');
		$pages = $this->db->pages;
		$big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\'?m=guestbook&c=guestbook&a=guestbook_send\', title:\''.L('all_send_message').'\', width:\'550\', height:\'300\'}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);', L('all_send_message'));
	...
	public function delete() {
		if((!isset($_GET['guestid']) || empty($_GET['guestid'])) && (!isset($_POST['guestid']) || empty($_POST['guestid']))) {
			...
			if(is_array($_POST['guestid'])){
				foreach($_POST['guestid'] as $messageid_arr) {
					//批量删除友情链接
